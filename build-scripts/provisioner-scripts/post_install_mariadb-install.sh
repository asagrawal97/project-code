...
sudo chown -R vagrant:vagrant ~/2021-team04r

# Using sed to replace variables in the scripts with the ENV variables passed
sed -i "s/\$ACCESSFROMIP/127.0.0.1/g" ~/2021-team04r/sprint-03/code/db-samples/*.sql
sed -i "s/\$USERPASS/$USERPASS/g" ~/2021-team04r/sprint-03/code/db-samples/*.sql
sed -i "s/\$MMIP/127.0.0.1/g" ~/2021-team04r/sprint-03/code/db-samples/*.sql
# This script will create the database named posts in the mariadb server
sudo mysql -u root < ~/2021-team04r/sprint-03/code/db-samples/create-database.sql
# This script will create the table named comments
sudo mysql -u root < ~/2021-team04r/sprint-03/code/db-samples/create-table.sql
# This script will insert 3 sample records to the table
sudo mysql -u root < ~/2021-team04r/sprint-03/code/db-samples/insert-records.sql
